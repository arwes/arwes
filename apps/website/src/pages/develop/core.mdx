...
export default props => <Page headProps={{ title: 'Core' }} {...props} />;
...
It depends on the [design](/develop/design), [animator](/develop/animator),
and [sounds](/develop/bleeps) packages.

> Since the components are still development, APIs are most likely to change
over the releases. It is not recommended to use them for real/production
projects yet.
...
```bash
npm install @arwes/design @arwes/animator @arwes/bleeps @arwes/core
```
...
See: [playground app](/play/p/core/StylesBaseline/basic).

## Example

A similar example as the sandbox [Figure basic](/play/p/core/Figure/basic)
can be replicated in a local project like:

```tsx
import React, { FC, useState, useEffect } from 'react';
import ReactDOM from 'react-dom';
import { AnimatorGeneralProvider, Animator } from '@arwes/animator';
...
// These are some examples assets URLs.
