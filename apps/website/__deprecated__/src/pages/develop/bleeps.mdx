...
export default props => <Page headProps={{ title: 'Sounds' }} {...props} />;
...
It goes hand in hand with the [animator tools](/develop/animator), though
it is not a dependency.

It is advised to follow the [Sound Accessibility](/develop/bleeps/accessibility)
recommendations.
...
the [animator tools](/develop/animator).
...
and to [categories of bleeps](/develop/bleeps/categories).
...
import { BleepsProvider, BleepsAudioSettings } from '@arwes/bleeps';
...
```tsx
import React, { FC } from 'react';
import { BleepsProvider, BleepsPlayersSettings } from '@arwes/bleeps';
...
```tsx
import React, { FC } from 'react';
import { BleepsProvider, BleepsSettings } from '@arwes/bleeps';
...
The bleeps can also have [categories](/develop/bleeps/categories).
...
```arwes_sandbox
const SOUND_CLICK_URL = 'https://arwes.dev/assets/sounds/click.mp3';
...
You can see and play with more examples in the [Play app](/play/p/sounds/BleepsProvider/basic).
