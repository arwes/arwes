...
export default props => <Page headProps={{ title: 'Nesting Animators' }} {...props} />;
...
flow in `parallel`. This behaviour can be changed by defining an [animator manager](/develop/animator/managers).
...
are transitioned in `parallel`. This can be configured. See [animator managers](/develop/animator/managers).
