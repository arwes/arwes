...
export default props => <Page headProps={{ title: 'Animation' }} {...props} />;
...
The animator tools require the [React](https://reactjs.org) component tree to
allow the dynamic communication between components.

It goes hand in hand with the [sounds tools](/develop/bleeps), though it is not
a dependency.
...
```bash
npm install @arwes/animator
```

The animator management tooling requires React v17 as a peer-dependency.
...
import { AnimatorRef, AnimatorClassSettings, withAnimator, useAnimator } from '@arwes/animator';
...
const ButtonComponent: FC = props => {
  ...
  animator.setupAnimateRefs(buttonRef);
...
[Nesting Animators](/develop/animator/nesting).
...
```tsx
import { AnimatorInstanceSettings, ... } from '@arwes/animator';
...
When [nesting animators](/develop/animator/nesting), the children nodes will
...
import { AnimatorGeneralProvider, AnimatorGeneralProviderSettings } from '@arwes/animator';
...
You can see and play with more examples in the [Play app](/play/p/animator/withAnimator/basic).
